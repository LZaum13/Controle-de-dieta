...
def cadastrar_usuario():
    ...
    while True:
        ...
        if idade > 0:
            ...
            print(
                "Erro: Idade inválida. Por favor, digite um número positivo.")
    while True:
        peso = obter_input_numerico("Peso (kg): ")
        if peso > 0:
            break
        else:
            print("Erro: Peso inválido. Por favor, digite um número positivo.")
    while True:
        ...
        if altura > 0:
            ...
            print(
                "Erro: Altura inválida. Por favor, digite um número positivo.")
    while True:
        objetivo = input(
            "Qual o seu objetivo (Ex: Emagrecer, Manter ou Ganhar Massa)? "
        ).upper()
        if objetivo in ["EMAGRECER", "MANTER", "GANHAR MASSA"]:
            break
        else:
            print(
                "Erro: Entrada inválida. Por favor, digite corretamente seu objetivo"
            )
...
def registrar_refeicao():
    ...
    nome_refeicao = input(
        "Qual o nome da refeição (ex: Café da Manhã, Almoço)? ")
    ...
    while True:
        limpar_tela()
        print(
            f"Adicionando alimentos para '{nome_refeicao}' de {usuario_selecionado.nome}"
        )
        print("\nSelecione um alimento da biblioteca:")

        alimento_selecionado = selecionar_item(biblioteca_alimentos,
                                               "alimento")
        ...
        try:
            quantidade = float(
                input(
                    f"Digite a quantidade de '{alimento_selecionado.nome}' em gramas: "
                ))
        ...
        continuar = input(
            "Deseja adicionar outro alimento a esta refeição? (s/n): ").lower(
            )
        if continuar != 's':
            break
...
def calcular_tmb_get():
    ...
    print(
        "5. Extremamente Ativo (exercício muito pesado, trabalho físico, 7 dias/semana)"
    )

    fatores_atividade = {'1': 1.2, '2': 1.375, '3': 1.55, '4': 1.725, '5': 1.9}
    ...
    metricas = usuario_selecionado.calcular_metricas_energeticas(
        fator_selecionado)
    ...
    if meta_sugerida > 0:
        ...
        confirmar = input(
            f"Deseja definir esta meta para {usuario_selecionado.nome}? (s/n): "
        ).lower()
        if confirmar == 's':
